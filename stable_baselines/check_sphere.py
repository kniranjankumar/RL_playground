...
the_path = "/home/niranjan/Projects/vis_inst/experiments/sphere_arm/2b_2a_10k_constrained_no_flip_resized"
...
class NetworkVecEnv(SubprocVecEnv):
    ...
    def run_rollouts(self, num_eps, policy=None):
        ...
        obs_list, act_list, mass_list, mu_list = [], [], [], []
        for i in tqdm(range(num_eps)):
            ...
            rollout_mu.append(np.array(mu_list[0].copy()))
            obs_list, act_list, mass_list, mu_list = [], [], [], []
        return np.array(rollout_obs), np.array(rollout_act), np.array(rollout_mass), np.array(rollout_mu)
    ...
    def train(self, num_eps, policy=None, is_fresh=True):
        ...
        if policy is not None or is_fresh:
            ...
            rollout_obs, rollout_act, rollout_mass, rollout_mu = self.run_rollouts(num_eps, policy)
            if policy is None:
                np.save(data_path + '/obs.npy', rollout_obs)
                np.save(data_path + '/act.npy', rollout_act)
                np.save(data_path + '/mass.npy', rollout_mass)
                np.save(data_path + '/mu.npy', rollout_mu)
        ...
        error = 0
        error = self.model.feedable_train(rollout_obs, rollout_act, rollout_mass, 1000000, self.graph, batch_size=16)

...
class NetworkVecEnv(SubprocVecEnv):
    def __init__(self, env_fns1,predictor_type, reward_type, path, reward_scale=1, num_steps=3, use_mass_distribution=False):
        ...
        self.sess = tf.Session(graph=self.predictor_graph)
        self.model = self.FCModel(self.path, self.sess, graph=self.predictor_graph, num_steps=num_steps, act_dim=self.action_space.shape[0],
    ...
    class FCModel:
        def __init__(self, path, sess, graph, num_steps, act_dim, mass_dim, obs_dim, mass_range, model_type='LSTM', use_mass_distribution=False):
            self.path = path
        ...
        def setup_feedable_training(self, lr=1e-1, loss='huber', is_init_all=True):
            with self.graph.as_default():
                # self.sess = sess
                if loss=='huber':
    ...
    def train(self, num_eps, data_path, save_dir, policy=None, is_fresh=True,lr=1, steps=500000):
        with self.predictor_graph.as_default():
